    ...
    private void txtTimKiemCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_txtTimKiemCaretUpdate
        ...
        if (cboTimkiem.getSelectedIndex() == 0) {
            ...
            for (Person bean : list) {
                ...
                Department department = departmentBN.getDepartmentByID(bean.getDepartmentName());
                Object[] rows = {bean.getFirstName(), bean.getLastName(), enterprise, department, bean.getPosition(), bean.getPersonID(), bean.getImage()};
                model.addRow(rows);
            }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (button == buttonDelete) {
            ...
            if (i != -1) {
                ...
                if (tc3 != null) {
                    if (tc3.getObs().isEmpty()) {
                        tc3.addObserver();
                    }
                    tc3.setChanged();
                    tc3.notifyObservers(null);
                }
            }
...
//            String image = listPersonPanel.getTableListP().getValueAt(row, 6).toString().trim();
//
//            bean = new Person(id, namef, namel, cty, bophan, chucvu, image);
...
//            this.open();
    ...
    public void mouseEvent(MouseEvent e) {
        ...
        PersonTopComponent personTopComponent = (PersonTopComponent) WindowManager.getDefault().findTopComponent("PersonTopComponent");
        for (int i = 0; i < rows; i++) {
            if (i == slectedRow) {
                if (e.getClickCount() == 2) {
                    if (buttonEdit.isEnabled() == false) {
                        String strRow = listPersonPanel.getTableListP().getValueAt(i, 5).toString().trim();
                        Person bean = personBN.getPersonByID(strRow);
                        if (bean != null) {
                            Enterprise enterprise = enterpriseBN.getEnterpriseByID(bean.getEnterpriseID());  // Lấy ra 1 Enterprise theo mã
                            Department department = departmentBN.getDepartmentByID(bean.getDepartmentName());
                            personTopComponent.getPersonPanel().getTabelPerson().setModel(new javax.swing.table.DefaultTableModel(
                                    new Object[][]{
                                        {"Họ (first name)", bean.getFirstName()},
                                        {"Tên (last name)", bean.getLastName()},
                                        {"Công ty", enterprise},
                                        {"Bộ phận", department},
                                        {"Chức vụ", bean.getPosition()},
                                        {"Mã Person", bean.getPersonID()},
                                        {"Ảnh", bean.getImage()}
                                    },
                                    new String[]{
                                        "", ""
                            ...
                            personCell.getTxtFName().setText(bean.getFirstName());
                            personCell.getTxtLName().setText(bean.getLastName());
                            personCell.getTxtIdP().setText(bean.getPersonID());
                            if(enterprise!=null){
                                 personCell.resetCombobox(enterprise);
                            }                          
                    ...
                    setupTable();
                    personTopComponent.requestActive();
                }
