...
public final class ListOperationTopComponent extends TopComponent implements ActionListener, MouseListener {
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addComponent(txtTongThu, javax.swing.GroupLayout.PREFERRED_SIZE, 168, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addContainerGap())
    ...
    private void txtTimKiemCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_txtTimKiemCaretUpdate
        ...
        if (cboTimkiem.getSelectedIndex() == 0) {
            String[] header = {"Tên nghiệp vụ", "Mã nghiệp vụ", "Ngày tháng năm", "Sản phảm dịch vụ", "Công ty",
                "Bộ phận (Phòng)", "Người chịu trách nhiệm", "Phân loại", "Số lượng", "Đơn giá", "Tổng giá", "Đơn vị tính", "Đơn vị đo"};
            model = new DefaultTableModel(header, 0);
            List<Operation> list = new ArrayList<Operation>();
            list = operationBN.filterOperationsByName(strTim);
            for (Operation bean : list) {
                Enterprise enterprise = enterpriseBN.getEnterpriseByID(bean.getEnterpriseID());  // Lấy ra 1 Enterprise theo mã
                Product product = productBN.getProductByID(bean.getProductID());
                Department department = departmentBN.getDepartmentByID(bean.getDepartmentID());
                Person person = personBN.getPersonByID(bean.getPersonID());

                // Đưa các Enterprise, Product, Department, Person vào table
                Object[] rows = {bean.getOperationName(), bean.getOperationID(), bean.getDateTime(), product,
                    enterprise, department, person, bean.getClassification(), String.valueOf(bean.getNumber()),
                    String.valueOf(bean.getUnitPrice()), String.valueOf(bean.getSumPrice()), bean.getCalculationUnit(), bean.getUnitOfMeasure()};
                model.addRow(rows);
            }
    ...
    public void loadData() {
        ...
        for (Operation bean : list) {
            Enterprise enterprise = enterpriseBN.getEnterpriseByID(bean.getEnterpriseID());
            Product product = productBN.getProductByID(bean.getProductID());
            Department department = departmentBN.getDepartmentByID(bean.getDepartmentID());
            Person person = personBN.getPersonByID(bean.getPersonID());
            Object[] rows = {bean.getOperationName(), bean.getOperationID(), bean.getDateTime(), product,
...
//            String dvtinh = listOperationPanel.getTableListO().getValueAt(row, 11).toString().trim();
//            String dvdo = listOperationPanel.getTableListO().getValueAt(row, 12).toString().trim();
//
//            bean = new Operation(id, name, day, spdv, cty, bophan, person, phanloai,
//                    Integer.parseInt(soluong), Integer.parseInt(dongia), Integer.parseInt(tonggia), dvtinh, dvdo);
...
//            this.open();
    ...
    public void mouseEvent(MouseEvent e) {
        ...
        OperationTopComponent operationTopComponent = (OperationTopComponent) WindowManager.getDefault().findTopComponent("OperationTopComponent");
        for (int i = 0; i < rows; i++) {
            if (i == slectedRow) {
                if (e.getClickCount() == 2) {
                        ...
                        if (bean != null) {
                            ...
                            operationTopComponent.getOperationPanel().getTableO().setModel(new javax.swing.table.DefaultTableModel(
                                    new Object[][]{
                                        ...
                                        {"Sản phảm dịch vụ", product},
                                        {"Công ty", enterprise},
                                        {"Bộ phận (Phòng)", department},
                                        {"Người chịu trách nhiệm", person},
                                        ...
                                        {"Đơn vị tính", bean.getCalculationUnit()},
                                        {"Đơn vị đo", bean.getUnitOfMeasure()}
                                    },
                                    new String[]{
                                        "", ""
                                    }) {
                            ...
                            operationCell.getTxtDonviTinh().setText(bean.getCalculationUnit());
                            if (enterprise != null) {
                                operationCell.resetCombobox(enterprise);
                            }
                    ...
                    setupTable();
                    operationTopComponent.requestActive();
                }
            }
        }
    }
