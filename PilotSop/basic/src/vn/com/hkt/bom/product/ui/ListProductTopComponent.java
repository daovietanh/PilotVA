    ...
    private void txtTimKiemCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_txtTimKiemCaretUpdate
        ...
        if (cboTimkiem.getSelectedIndex() == 0) {
            ...
            for (Product bean : list) {
                ...
                Product product = productBN.getProductByID(bean.getProductgroups());
                ...
                Object[] rows = {bean.getProductName(), bean.getProductID(), enterprise, department, person, product};
                model.addRow(rows);
            }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (button == buttonDelete) {
            ...
            if (i != -1) {
                ...
                if (tc != null) {
                    if (tc.getObs().isEmpty()) {
                        tc.addObserver();
                    }
                    tc.setChanged();
                    tc.notifyObservers(null);
                }
...
//            this.open();
    ...
    public void mouseEvent(MouseEvent e) {
        ...
        ProductTopComponent productTopComponent = (ProductTopComponent) WindowManager.getDefault().findTopComponent("ProductTopComponent");
            ...
            if (i == slectedRow) {
                if (e.getClickCount() == 2) {
                    if (buttonEdit.isEnabled() == false) {
                        String strRow = listProductPanel.getTableListPr().getValueAt(i, 1).toString().trim();
                        Product bean = productBN.getProductByID(strRow);
                        if (bean != null) {
                            Enterprise enterprise = enterpriseBN.getEnterpriseByID(bean.getEnterpriseID());  // Lấy ra 1 Enterprise theo mã
                            Product product = productBN.getProductByID(bean.getProductgroups());
                            Department department = departmentBN.getDepartmentByID(bean.getDepartmentID());
                            ...
                            productTopComponent.getProductPanel().getTableProduct().setModel(new javax.swing.table.DefaultTableModel(
                                    new Object[][]{
                                        {"Tên sản phẩm dịch vụ", bean.getProductName()},
                                        {"Mã sản phẩm dịch vụ", bean.getProductID()},
                                        {"Công ty", enterprise},
                                        {"Bộ phân (Phòng)", department},
                            ...
                            productCell.getTxtNamePro().setText(bean.getProductName());
                            productCell.getTxtIdPro().setText(bean.getProductID());
                            if (enterprise != null) {
                                productCell.resetCombobox(enterprise);
                            }
                    ...
                    setupTable();
                    productTopComponent.requestActive();
                }
